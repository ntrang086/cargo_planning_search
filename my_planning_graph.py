...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            pgn_a = PgNode_a(a)
            if pgn_a.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(pgn_a)
                pgn_a.parents.update(pgn_a.prenodes)
                for pgn_s in pgn_a.parents:
                    pgn_s.children.add(pgn_a)
    ...
    def add_literal_level(self, level):
        ...
        for pgn_a in self.a_levels[level - 1]:
            self.s_levels[level].update(pgn_a.effnodes)
            pgn_a.children.update(pgn_a.effnodes)
            for pgn_s in pgn_a.children:
                pgn_s.parents.add(pgn_a)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        the preconditions of one action is mutex with a precondition of the
        other action.
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        return all(pgn_a1.is_mutex(pgn_a2) for pgn_a1 in node_s1.parents for pgn_a2 in node_s2.parents)
    ...
    def h_levelsum(self) -> int:
        ...
        goals_to_check = [g for g in self.problem.goal]
        for level, s_set in enumerate(self.s_levels):
            for s in s_set:
                if s.symbol in goals_to_check and s.is_pos:
                    level_sum += level
                    goals_to_check.remove(s.symbol)
        return level_sum
